...
#include "ProjectDamageExecution.h"
#include "Project/GamePlay/AbilitySystem/Attributes/ProjectHealthSet.h"
#include "Project/GamePlay/AbilitySystem/Attributes/ProjectCombatSet.h"
#include "Engine/World.h"

#include UE_INLINE_GENERATED_CPP_BY_NAME(ProjectDamageExecution)
...
struct FDamageStatics
{
	...
	FDamageStatics()
	{
		BaseDamageDef = FGameplayEffectAttributeCaptureDefinition(UProjectCombatSet::GetBaseDamageAttribute(), EGameplayEffectAttributeCaptureSource::Source, true);
	}
};
...
UProjectDamageExecution::UProjectDamageExecution()
{
	RelevantAttributesToCapture.Add(DamageStatics().BaseDamageDef);
}

void UProjectDamageExecution::Execute_Implementation(const FGameplayEffectCustomExecutionParameters& ExecutionParams, FGameplayEffectCustomExecutionOutput& OutExecutionOutput) const
{
#if WITH_SERVER_CODE
	...
	if (HitActor)
	{
		// UProjectTeamSubsystem* TeamSubsystem = HitActor->GetWorld()->GetSubsystem<UProjectTeamSubsystem>();
	...
	// if (const IProjectAbilitySourceInterface* AbilitySource = TypedContext->GetAbilitySource())
	// {
	...
	if (DamageDone > 0.0f)
	{
		...
		OutExecutionOutput.AddOutputModifier(FGameplayModifierEvaluatedData(UProjectHealthSet::GetDamageAttribute(), EGameplayModOp::Additive, DamageDone));
	}
