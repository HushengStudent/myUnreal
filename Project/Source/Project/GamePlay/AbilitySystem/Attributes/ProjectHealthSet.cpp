...
UE_DEFINE_GAMEPLAY_TAG(TAG_Project_Damage_Message, "Project.Damage.Message");
...
void UProjectHealthSet::PostGameplayEffectExecute(const FGameplayEffectModCallbackData& Data)
{
	...
	if (!bIsDamageFromSelfDestruct &&
		(Data.Target.HasMatchingGameplayTag(ProjectGameplayTags::Cheat_GodMode) || Data.Target.HasMatchingGameplayTag(ProjectGameplayTags::Cheat_UnlimitedHealth) ))
	{
		MinimumHealth = 1.0f;
	}
#endif
	...
	if (Data.EvaluatedData.Attribute == GetDamageAttribute())
	{
		if (Data.EvaluatedData.Magnitude > 0.0f)
		{
			...
			Message.Verb = TAG_Project_Damage_Message;
...
void UProjectHealthSet::PostAttributeChange(const FGameplayAttribute& Attribute, float OldValue, float NewValue)
{
	...
	if (Attribute == GetMaxHealthAttribute())
	{
		...
		{
			UProjectAbilitySystemComponent* ProjectASC = GetProjectAbilitySystemComponent();
			check(ProjectASC);
			ProjectASC->ApplyModToAttribute(GetHealthAttribute(), EGameplayModOp::Override, NewValue);
		}
	}
